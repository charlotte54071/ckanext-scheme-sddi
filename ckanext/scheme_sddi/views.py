...
def restricted_request_access_form(package_id, resource_id, data=None, errors=None, error_summary=None):
    ...
    user_id = tk.g.user
    if not user_id:
        tk.abort(401, _('Access request form is available to logged in users only.'))
    ...
    if not data:
        ...
        try:
            ...
            for resource in resources:
                ...
                tk.abort(404, 'Dataset resource not found')
            # get mail
            ...
            tk.abort(404, tk._(u'Exception retrieving dataset for the form ({})').format(str(e)))
        except Exception:
            tk.abort(404, tk._('Unknown exception retrieving dataset for the form'))
...
def _send_request_mail(data):
    ...
    try:
        ...
        resource_link = tk.url_for(
            action='resource_read',
            id=data.get('package_name'),
            resource_id=data.get('resource_id'))

        resource_edit_link = tk.url_for(
        ...
        extra_vars = {
            ...
            'resource_link': tk.config.get('ckan.site_url') + resource_link,
            'resource_edit_link': tk.config.get('ckan.site_url') + resource_edit_link,
            'package_name': data.get('resource_name', ''),
            'message': data.get('message', ''),
            'admin_email_to': tk.config.get('email_to', 'email_to_undefined')}
...
def _send_request(context):
    ...
    try:
        ...
        tk.abort(404, _('Exception retrieving dataset to send mail'))
...
def _get_contact_details(pkg_dict):
    ...
    # CKAN instance Admin
    if not contact_email:
        contact_email = tk.config.get('email_to', 'email_to_undefined')
